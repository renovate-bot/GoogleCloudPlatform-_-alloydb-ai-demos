...
from src.db.quantity_forecast import quantity_forecast, get_historical_data_from_db
import asyncio
import pandas as pd
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    It uses the existing singleton `alloydb_client` to manage the engine's lifecycle.

    Args:
        app (FastAPI): The FastAPI application instance.
    """
    logger.info("Application startup: Initializing resources...")
    # The engine is created when alloydb_client is instantiated.
    # We can perform any startup checks here if needed.
    yield
    logger.info("Application shutdown: Disposing database engine...")
    alloydb_client.engine.dispose()


app = FastAPI(
    title="Inventory Management AlloyDB API",
    description="An API for Agentic inventory management using AlloyDB database.",
    lifespan=lifespan,
)
...
async def websocket_status(websocket: WebSocket):
    ...
    This endpoint accepts connections from frontend clients and keeps them
    alive. Messages are broadcasted via `broadcast_status`.
...
class QuantityForecast(BaseModel):
    sku: str
    store_id: int
    horizon_days: int
...
def root(request: Request):
    ...
    <html>
        ...
        <body>
            <h2>Welcome to the Inventory Management Alloydb FastAPI Service!</h2>
...
def series_to_records(s: pd.Series, value_col: str, date_col: str = "date"):
    if s is None or s.empty:
        return []
    df = s.rename(value_col).to_frame().reset_index()
    # rename index column to date_col
    df.rename(columns={df.columns[0]: date_col}, inplace=True)
    # format date
    df[date_col] = (
        pd.to_datetime(df[date_col]).dt.tz_localize(None).dt.strftime("%Y-%m-%d")
    )
    return df.to_dict(orient="records")


@app.post(
    "/dashboard/inventory/overview/historical_data",
    summary="To provide the historical data for quantity forecast",
    description="Returns the JSON which has the historical data",
    responses={500: {"description": "Internal server error"}},
)
@log_execution(is_api=True)
async def get_historical_data(
    request: HistoricalData, engine: Engine = Depends(get_engine)
):
    sku = request.sku
    store_id = request.store_id
    historical_data = get_historical_data_from_db(engine, sku, store_id)
    json_data = series_to_records(historical_data, value_col="quantity")
    return {"historical_data": json_data}


@app.post(
    "/dashboard/inventory/overview/quantity_forecast",
    summary="To provide the quantity forecast for low stock",
    description="Returns the JSON which has the forecast data for Low Stock items",
    responses={500: {"description": "Internal server error"}},
)
@log_execution(is_api=True)
async def get_quantity_forecast(
    request: QuantityForecast, engine: Engine = Depends(get_engine)
):
    sku = request.sku
    store_id = request.store_id
    horizon_days = request.horizon_days
    forecast_results = quantity_forecast(engine, sku, store_id, horizon_days)
...
async def retrive_recommendation(
    ...
    Delegates forecasting and recommendation logic to the `recommend_replenishment`
    agent coordinator, and normalizes the result into a JSON-friendly structure.
...
if __name__ == "__main__":
    # Host 0.0.0.0 makes the service reachable from outside the container/VM.
