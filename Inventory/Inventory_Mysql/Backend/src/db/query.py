...
async def inventory_overview(engine):
    """
    Generates an inventory overview DataFrame with status classification and
    base64-encoded image data for UI/JSON transport.

    Workflow:
        1) Query stock levels joined with products and stores.
        2) LATERAL-like logic (implemented via subqueries) selects the most recent
           product image (`pi`) or a default image (`dpi`).
        3) Classify each row into one of: Critical, Low, Optimal, Over Stock.
        4) Base64-encode image bytes for downstream rendering.
    ...
    Returns:
        pandas.DataFrame | str:
            - A DataFrame with columns:
              ...
              where `image_data` is base64-encoded.
            - On error, returns a string describing the failure.
    """
    # -------------------------------------------------------------------------
    # 1. Fetch data from the database by joining stock, product, and store tables.
    #    In MySQL, the "LATERAL" join pattern is simulated by derived tables /
    #    subqueries combined with LEFT JOINs (as crafted in your SQL).
    # -------------------------------------------------------------------------
    try:
        ...
        SELECT
                ...
                COALESCE(pi.image_data,
                        (SELECT dpi.image_data
                        FROM {MYSQL_TABLE_SCHEMA}.product_images AS dpi
                ...
                COALESCE(pi.image_url,
                        (SELECT dpi.image_url
                        FROM {MYSQL_TABLE_SCHEMA}.product_images AS dpi
                        WHERE dpi.is_default = TRUE
                        ORDER BY dpi.created_at DESC
                        LIMIT 1)) AS image_url
            FROM {MYSQL_TABLE_SCHEMA}.stock_levels AS sl
            JOIN {MYSQL_TABLE_SCHEMA}.products AS p
            ON sl.sku = p.sku
            JOIN {MYSQL_TABLE_SCHEMA}.stores AS s
            ON sl.store_id = s.store_id
            LEFT JOIN (
                SELECT sku, image_url, image_data
                FROM (
                    SELECT
                        sku,
                        image_url,
                    ...
                    FROM {MYSQL_TABLE_SCHEMA}.product_images
                ) AS ranked
                WHERE rn = 1
            ) AS pi
            ON pi.sku = p.sku;
        ...
        # Execute and convert results into a DataFrame; return empty DF when no rows.
        try:
        ...
        # ---------------------------------------------------------------------
        # Encode binary image payloads to base64 for safe transport.
...
async def recent_purchase_order(engine):
    ...
    po_query = text(
        ...
        FROM {MYSQL_TABLE_SCHEMA}.purchase_orders po
        JOIN {MYSQL_TABLE_SCHEMA}.suppliers s ON po.supplier_id = s.supplier_id
...
async def add_product(
    ...
    """
    Adds a new product (or updates existing via MySQL ON DUPLICATE KEY) and
    upserts stock levels for a store–SKU.

    Workflow:
        1) Insert into `{MYSQL_TABLE_SCHEMA}.products` with an image payload:
           - Prefer product-specific image for the SKU.
           - Fall back to 'DEFAULT' image when missing.
           - On duplicate key, selectively update non-empty title/category.
        2) Upsert into `{MYSQL_TABLE_SCHEMA}.stock_levels` for the provided store:
    ...
    try:
        with engine.connect() as connection:
            ...
            # - Update `title`/`category` only when new values are non-empty.
            # -----------------------------------------------------------------
            result = connection.execute(
                text(
                    f"""
                    INSERT INTO {MYSQL_TABLE_SCHEMA}.products (sku, title, category, image_data)
            ...
            if result.rowcount == 0:
                ...
                logger.info(f"Data inserted into table {MYSQL_TABLE_SCHEMA}.products!!")

            # -----------------------------------------------------------------
            # Upsert stock levels regardless of product insert outcome.
            # MySQL ON DUPLICATE KEY UPDATE uses VALUES() to set new data.
            # -----------------------------------------------------------------
            connection.execute(
                text(
                    f"""
                    INSERT INTO {MYSQL_TABLE_SCHEMA}.stock_levels (sku, store_id, on_hand, in_transit, safety_stock, reorder_point)
            ...
            logger.info(
                f"Data inserted/updated in table {MYSQL_TABLE_SCHEMA}.stock_levels!!"
            )
            connection.commit()
...
async def edit_product(
    ...
    Returns:
        str: Success message, or a message indicating no changes were applied when
        provided values match existing data.
    """
        ...
        with engine.begin() as connection:
            ...
            if store_id and any(
                ...
                stock_result = connection.execute(
                    text(
                        f"""
                    UPDATE {MYSQL_TABLE_SCHEMA}.stock_levels
            ...
            if stock_result.rowcount == 0:
                logger.info(
                    f"No changes were applied to SKU {sku} because all provided values match the existing data."
                )
                return f"No changes were applied to SKU {sku} because all provided values match the existing data."
            logger.info(f"Data updated to {MYSQL_TABLE_SCHEMA}.stock_levels")
...
async def edit_purchase_order_recommedation(
    ...
    Edits a purchase order recommendation by:
        - Updating the PO's supplier.
        - Updating the product–supplier mapping preference for the SKU.
        - Adjusting the line quantity.
    ...
    Args:
        ...
        sku: SKU whose supplier/quantity is being changed.
        new_supplier_id: Supplier id to assign to the PO and product mapping.
        new_quantity: New quantity to set for the SKU on the PO line.

    Returns:
        Dict[str, Any]: Success payload `{status, po_id}` or error dict with message.
    """
    ...
    try:
        with engine.begin() as connection:
            ...
            # result = connection.execute(
            #     text(f"SELECT supplier_id, name FROM {MYSQL_TABLE_SCHEMA}.suppliers ORDER BY name")
            # )
            ...
            connection.execute(
                text(
                    f"""
                UPDATE {MYSQL_TABLE_SCHEMA}.purchase_orders
            ...
            logger.info(f"Data updated to {MYSQL_TABLE_SCHEMA}.purchase_orders")

            # Update product-supplier preference for the SKU.
            connection.execute(
                text(
                    f"""
                    UPDATE {MYSQL_TABLE_SCHEMA}.product_suppliers
                    SET preferred = CASE WHEN supplier_id = :new_supplier_id THEN 1 ELSE 0 END
                    WHERE sku = :sku
                """
                ),
                {"new_supplier_id": new_supplier_id, "sku": sku},
            )
            logger.info(f"Data updated to {MYSQL_TABLE_SCHEMA}.product_supplier_id")

            # Update order line quantity for the given SKU within the PO.
            connection.execute(
                text(
                    f"""
                    UPDATE {MYSQL_TABLE_SCHEMA}.purchase_order_lines
            ...
            logger.info(f"Data updated to {MYSQL_TABLE_SCHEMA}.purchase_order_lines")

            # Recalculate total_amount based on updated line items.
            connection.execute(
                text(
                    f"""
                UPDATE {MYSQL_TABLE_SCHEMA}.purchase_orders
                SET total_amount = (
                    ...
                    FROM {MYSQL_TABLE_SCHEMA}.purchase_order_lines
                    WHERE po_id = :po_id
                )
                WHERE po_id = :po_id
            """
...
async def approve_po_recommendation(engine, po_id):
    ...
    try:
        with engine.begin() as connection:
            ...
            logger.info(
                f"Data updated to {MYSQL_TABLE_SCHEMA}.purchase_orders, Approval sent!!"
            )
            # trans.commit()
        return {"status": "success", "po_id": po_id}
    except Exception as e:
