...
async def recommend_replenishment(
    ...
    for o in options:
        ...
        # Skip when no demand is expected.
        if needed <= 0:
        ...
        # Initial quantity: respect MOQ, round up shortfall.
        qty = max(moq, ceil(needed))
        value = qty * cost

        # Apply single-PO value cap; adjust qty downward if needed.
        ...
        # Guard against zero quantity after capping.
        if qty <= 0:
