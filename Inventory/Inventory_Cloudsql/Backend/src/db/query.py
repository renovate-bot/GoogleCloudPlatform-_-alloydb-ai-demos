...
async def inventory_overview(engine):
    ...
    Workflow:
        ...
        2) LATERAL join the most recent product image (`pi`) or a default image (`dpi`).
    ...
    #      - `dpi`: default image when no specific SKU image exists.
    # -------------------------------------------------------------------------
    try:
        ...
        FROM {CLOUDSQL_TABLESCHEMA}.stock_levels sl
        JOIN {CLOUDSQL_TABLESCHEMA}.products p ON sl.sku = p.sku
        JOIN {CLOUDSQL_TABLESCHEMA}.stores s   ON sl.store_id = s.store_id

        -- Per-product image: take the most recent mapping for this SKU
        LEFT JOIN LATERAL (
            SELECT image_url, image_data
            FROM {CLOUDSQL_TABLESCHEMA}.product_images
            WHERE sku = p.sku
        ...
        try:
            ...
            # Handle query/connection exceptions distinctly (kept as print to match original code style).
            return {"error": f"Exception occured!! , {e}"}
        ...
        return {"error": f"Failed to get inventory overview: {e}"}
...
async def recent_purchase_order(engine):
    ...
    po_query = text(
        ...
        FROM {CLOUDSQL_TABLESCHEMA}.purchase_orders po
        JOIN {CLOUDSQL_TABLESCHEMA}.suppliers s ON po.supplier_id = s.supplier_id
...
async def add_product(
    ...
    Workflow:
        1) Insert into `{CLOUDSQL_TABLESCHEMA}.products` with an image payload:
        ...
        2) Upsert into `{CLOUDSQL_TABLESCHEMA}.stock_levels` for the provided store:
    ...
    try:
        with engine.connect() as connection:
            ...
            result = connection.execute(
                text(
                    f"""
                    INSERT INTO {CLOUDSQL_TABLESCHEMA}.products (sku, title, category, image_data)
            ...
            if result.rowcount == 0:
                ...
                logger.info(
                    f"Data inserted into table {CLOUDSQL_TABLESCHEMA}.products!!"
                )
            ...
            connection.execute(
                text(
                    f"""
                    INSERT INTO {CLOUDSQL_TABLESCHEMA}.stock_levels (sku, store_id, on_hand, in_transit, safety_stock, reorder_point)
            ...
            logger.info(
                f"Data inserted/updated in table {CLOUDSQL_TABLESCHEMA}.stock_levels!!"
            )
        ...
        with engine.begin() as connection:
            ...
            if store_id and any(
                ...
                stock_result = connection.execute(
                    text(
                        f"""
                    UPDATE {CLOUDSQL_TABLESCHEMA}.stock_levels
            ...
            if stock_result.rowcount == 0:
                logger.error(
                    f"Failed to update {CLOUDSQL_TABLESCHEMA}.stock_levels. No stock record found for SKU {sku}"
                )
                return f"Failed to update {CLOUDSQL_TABLESCHEMA}.stock_levels. No stock record found for SKU {sku}"
            logger.info(f"Data updated to {CLOUDSQL_TABLESCHEMA}.stock_levels")
            # trans.commit()
...
async def edit_purchase_order_recommedation(
    ...
    try:
        with engine.begin() as connection:
            ...
            # result = connection.execute(
            #     text(f"SELECT supplier_id, name FROM {CLOUDSQL_TABLESCHEMA}.suppliers ORDER BY name")
            # )
            ...
            connection.execute(
                text(
                    f"""
                UPDATE {CLOUDSQL_TABLESCHEMA}.purchase_orders
            ...
            logger.info(f"Data updated to {CLOUDSQL_TABLESCHEMA}.purchase_orders")

            # Update product-supplier mapping for the SKU.
            connection.execute(
                text(
                    f"""
                    UPDATE {CLOUDSQL_TABLESCHEMA}.product_suppliers
            ...
            logger.info(f"Data updated to {CLOUDSQL_TABLESCHEMA}.product_supplier_id")

            # Update order line quantity for the given SKU within the PO.
            connection.execute(
                text(
                    f"""
                    UPDATE {CLOUDSQL_TABLESCHEMA}.purchase_order_lines
            ...
            logger.info(f"Data updated to {CLOUDSQL_TABLESCHEMA}.purchase_order_lines")
            ...
            connection.execute(
                ...
                SET total_amount = (
                    ...
                    FROM {CLOUDSQL_TABLESCHEMA}.purchase_order_lines
                    WHERE po_id = :po_id
                )
                WHERE po_id = :po_id
            """
...
async def approve_po_recommendation(engine, po_id):
    ...
    try:
        with engine.begin() as connection:
            ...
            logger.info(
                f"Data updated to {CLOUDSQL_TABLESCHEMA}.purchase_orders, Approval sent!!"
            )
            # trans.commit()
        return {"status": "success", "po_id": po_id}
    except Exception as e:
